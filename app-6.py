...
TUTORIAL_TEXT = (
    "📖 راهنمای اتصال\n\n"
    "۱. اپلیکیشن مخصوص اتصال رو نصب کن (لینک دانلود رو پشتیبانی برات می‌فرسته)\n"
    "۲. فایل کانفیگی که براتون ارسال می‌شه رو داخل اپ وارد کن\n"
    "۳. روی دکمه اتصال بزن و از اینترنت آزاد لذت ببر!\n\n"
    "اگه توی هر مرحله مشکلی داشتی، از دکمه پشتیبانی زیر همین پیام استفاده کن."
)
...
def telegram_webhook():
    ...
    if "message" in update:
        ...
        # دستورات مخصوص ادمین
        if str(chat_id) == str(TELEGRAM_ADMIN_CHAT_ID) and "text" in msg:
            text = msg["text"].strip()
        ...
        if msg.get("text") == "/start":
            ...
            # اطلاع‌رسانی آیدی مخاطب جدید به ادمین
        ...
        if "photo" in msg:
            ...
            tg_send(
                ...
                f"برای ارسال سرور به این مشتری:\n/send {chat_id} <پیام سرور>"
            )
            tg_send(chat_id, "رسید شما دریافت شد ✅\nبعد از تایید پشتیبانی، اطلاعات سرور برات ارسال می‌شه.")
            return jsonify({"ok": True})

        # هر پیام دیگه‌ای: منو رو دوباره نشون بده
    ...
    if "callback_query" in update:
        ...
        if data == "menu":
            tg_send(chat_id, "منوی اصلی:", keyboard=main_menu_keyboard())

        elif data == "plans":
            tg_send(chat_id, "یکی از پلن‌ها رو انتخاب کن (همه نامحدود):", keyboard=plans_keyboard())

        elif data == "tutorial":
            tg_send(chat_id, TUTORIAL_TEXT, keyboard=[[{"text": "⬅️ بازگشت به منو", "callback_data": "menu"}]])
